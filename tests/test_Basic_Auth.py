...
@allure.story("Авторизация через Basic Auth")
@allure.title("Успешный вход с валидными учетными данными")
@allure.severity(allure.severity_level.CRITICAL)


def test_basic_auth(main_tab):
    with allure.step("Переход на страницу basic auth"):
        auth_url = 'https://admin:admin@the-internet.herokuapp.com/basic_auth'
        main_tab.goto(auth_url)
        main_tab.wait_for_load_state('networkidle')

    with allure.step("Формирование URL с авторизацией и переход"):
        success = main_tab.locator('//p[contains(text(), "Congratulations")]')
